    ...
    ExponentialNoError
{
    ...
    event NewVAIVaultInfo(
        address vault_,
        uint releaseStartBlock_,
        uint releaseInterval_
    );
    ...
    event NewVenusVAIVaultRate(
        uint oldVenusVAIVaultRate,
        uint newVenusVAIVaultRate
    );
    ...
    event NewComptrollerLens(
        address oldComptrollerLens,
        address newComptrollerLens
    );
    ...
    function exitMarket(address vTokenAddress) external returns (uint) {
        ...
        (uint oErr, uint tokensHeld, uint amountOwed, ) = vToken
    ...
    function mintAllowed(
        ...
        Exp memory exchangeRate = Exp({
            mantissa: VToken(vToken).exchangeRateStored()
        });
    ...
    function liquidateBorrowAllowed(
        ...
        if (
            liquidatorContract != address(0) && liquidator != liquidatorContract
        ) {
        ...
        uint borrowBalance;
        if (address(vTokenBorrowed) != address(vaiController)) {
            borrowBalance = VToken(vTokenBorrowed).borrowBalanceStored(
                borrower
            );
        } else {
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        (uint err, uint liquidity, uint shortfall) = comptrollerLens
    ...
    function liquidateCalculateSeizeTokens(
        ...
        (uint err, uint seizeTokens) = comptrollerLens
    ...
    function _setCollateralFactor(
        ...
        if (
            ...
            oracle.getUnderlyingPrice(address(vToken)) == 0
        ) {
            return
                fail(
                    Error.PRICE_ERROR,
    ...
    function _setLiquidationIncentive(
        ...
        require(
            newLiquidationIncentiveMantissa >= 1e18,
    ...
    //             0
    //         );
    //         venusAccrued[holder] = grantXVSInternal(
    //             holder,
    //             venusAccrued[holder],
    //             shortfall,
    //             collateral
    ...
    //     require(
    //         VBep20Interface(getXVSVTokenAddress()).mintBehalf(user, amount) ==
    //             uint(Error.NO_ERROR),
    ...
    function setMintedVAIOf(
        ...
        // Pausing is a very serious situation - we revert to sound the alarms
        require(
            !mintVAIGuardianPaused && !repayVAIGuardianPaused,
            "VAI is paused"
        );
