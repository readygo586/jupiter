    ...
    ExponentialNoError
{
    ...
    function liquidateCalculateSeizeTokens(
        ...
        uint priceBorrowedMantissa = Comptroller(comptroller)
            .oracle()
            .getUnderlyingPrice((vTokenBorrowed));
        uint priceCollateralMantissa = Comptroller(comptroller)
            .oracle()
            .getUnderlyingPrice((vTokenCollateral));
        ...
        numerator = mul_(
            Exp({
                mantissa: Comptroller(comptroller)
    ...
    function getHypotheticalAccountLiquidity(
        ...
        for (uint i = 0; i < assetsCount; ++i) {
            ...
            (, uint collateralFactorMantissa, ) = Comptroller(comptroller)
                .markets(address(asset));
            vars.collateralFactor = Exp({mantissa: collateralFactorMantissa});
            vars.exchangeRate = Exp({mantissa: vars.exchangeRateMantissa});

            // Get the normalized price of the asset
            vars.oraclePriceMantissa = Comptroller(comptroller)
                .oracle()
        ...
        VAIControllerInterface vaiController = Comptroller(comptroller)
            .vaiController();
